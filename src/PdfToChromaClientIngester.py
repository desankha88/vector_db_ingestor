...
class PDFToChromaIngester:
    def __init__(self, collection_name: str = "airline_travel_docs"):
        """Initialize the PDF ingester for ChromaDB"""
        self.collection_name = collection_name

        # Initialize ChromaDB client
        self.client = chromadb.HttpClient(host="chroma", port = 8000, settings=Settings(allow_reset=True, anonymized_telemetry=False))

        # Delete existing collection if it exists (fresh start)
        try:
            self.client.delete_collection(name=collection_name)
            logger.info(f"Deleted existing collection: {collection_name}")
        except Exception as e: # Catch a more specific exception or at least log the type
            logger.warning(f"Could not delete collection '{collection_name}' (might not exist): {e}")
            pass # Only pass if you are sure it's an expected failure (e.g., collection not found)
